...
function formatTimeData(timeData) {
  let formattedTime = '';
  if (timeData.days > 0) {
    formattedTime += 'Days: ' + timeData.days + ' ';
  }
  if (timeData.hours > 0 || timeData.days > 0) {
    formattedTime += 'Hours: ' + String(timeData.hours).padStart(2, '0') + ' ';
  }
  if (timeData.minutes > 0 || timeData.hours > 0 || timeData.days > 0) {
    formattedTime += 'Minutes: ' + String(timeData.minutes).padStart(2, '0') + ' ';
  }
  formattedTime += 'Seconds: ' + String(timeData.seconds).padStart(2, '0');
